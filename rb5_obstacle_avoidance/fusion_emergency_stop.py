...
Responsibilities:
- Fuse camera (long range navigation) and lidar (short range safety)
- Listen to ObstacleChecker override commands (STOP / REVERSE / NORMAL)
- Send final velocity + steering via UDP (single source of truth)

IMPORTANT:
- This is the ONLY node that sends commands to motors (via UDP)
"""
...
class MasterNavigator(Node):
    ...
    def __init__(self):
        ...
        self.drive_mode = "NORMAL"   # NORMAL / STOP / REVERSE

        self.get_logger().info("Master Navigator with emergency stop - Fusion node started")
    ...
    def lidar_cb(self, msg):
        ...
        # Ignore rear hemisphere and invalid values
    ...
    def cam_cb(self, msg):
        ...
        # Steering from free space center
        weights = distances
    ...
    def publish_udp(self):
        ...
        if self.drive_mode == "STOP":
            telemetry = {"velocity": 0.0, "steer": 0.0}
            sock.sendto(json.dumps(telemetry).encode(), (UDP_IP, UDP_PORT))
            self.get_logger().error("OVERRIDE: STOP")
            return

        if self.drive_mode == "REVERSE":
            telemetry = {"velocity": REVERSE_SPEED, "steer": 0.0}
            sock.sendto(json.dumps(telemetry).encode(), (UDP_IP, UDP_PORT))
            self.get_logger().warn("OVERRIDE: REVERSE")
            return
        # ----------------------------------
